      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="card p-6">
          ...
          {summary.by_gateway.map(s => (
            <BarRow key={s.gateway_id} label={s.gateway_id.split("-").map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ") + " Gateway"} value={s.cost} maxValue={maxGatewayCost} color="bg-cyan-500" />
          ))}
        </div>
